...
class YAMLMap extends Collection {
  ...
  get(key, keepScalar) {
    ...
    return (_a = !keepScalar && isScalar$1(node) ? node.value : node) != null ? _a : void 0;
  }
  has(key) {
...
(function(module, exports) {
  ...
  var argsTag = "[object Arguments]", arrayTag = "[object Array]", boolTag2 = "[object Boolean]", dateTag = "[object Date]", errorTag = "[object Error]", funcTag = "[object Function]", genTag2 = "[object GeneratorFunction]", mapTag = "[object Map]", numberTag = "[object Number]", objectTag = "[object Object]", promiseTag = "[object Promise]", regexpTag = "[object RegExp]", setTag = "[object Set]", stringTag = "[object String]", symbolTag = "[object Symbol]", weakMapTag = "[object WeakMap]";
  ...
  var DataView = getNative(root, "DataView"), Map2 = getNative(root, "Map"), Promise = getNative(root, "Promise"), Set2 = getNative(root, "Set"), WeakMap = getNative(root, "WeakMap"), nativeCreate = getNative(Object, "create");
  var dataViewCtorString = toSource(DataView), mapCtorString = toSource(Map2), promiseCtorString = toSource(Promise), setCtorString = toSource(Set2), weakMapCtorString = toSource(WeakMap);
  ...
  if (DataView && getTag(new DataView(new ArrayBuffer(1))) != dataViewTag || Map2 && getTag(new Map2()) != mapTag || Promise && getTag(Promise.resolve()) != promiseTag || Set2 && getTag(new Set2()) != setTag || WeakMap && getTag(new WeakMap()) != weakMapTag) {
  ...
  function isFunction(value) {
    ...
    return tag == funcTag || tag == genTag2;
  }
...
const arrayToTag = (array) => array.map((folder) => folder ? `[${folder}]` : "").join("");
const genTag = (workspace) => {
  if (!workspace)
    return ["", []];
...
const genMultipleTag = (sortedTagWorkspaces) => {
  const tags = [];
  sortedTagWorkspaces.forEach(([workspace]) => {
    const [, subTag] = genTag(workspace);
    if (subTag == null ? void 0 : subTag.length) {
      if (subTag.length === 2) {
...
var genTag$1 = (workspaces) => {
  var _a;
  const changedFileMap = gitChangedFiles();
  ...
  let tag;
  if (!isMultiple) {
    [tag] = genTag((_a = tagWorkspaces[0]) == null ? void 0 : _a[0]);
  } else {
    const sortedTagWorkspaces = [...tagWorkspaces].sort(([, prevWeight], [, nextWeight]) => nextWeight - prevWeight);
    tag = genMultipleTag(sortedTagWorkspaces);
  }
  return tag;
};
const longest = require("longest");
const commonMsg = (options2) => {
...
const monoMsg = (options2, workspaces) => {
  const { types: types2, defaultType: defaultType2 } = options2;
  const length = longest(Object.keys(types2 != null ? types2 : {})).length + 1;
  var choices = map(types2 != null ? types2 : {}, function(type, key) {
    return {
      name: (key + ":").padEnd(length) + " " + type.description,
...
const setCommit = (options2) => {
  ...
  if (workspaces.length === 0)
    return commonMsg(options2);
  return monoMsg(options2, workspaces);
};
