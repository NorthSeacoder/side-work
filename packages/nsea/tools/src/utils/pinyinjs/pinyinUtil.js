 ...
 * @last 2016-09-29
 */
// ref: https://github.com/sxei/pinyinjs
/* eslint-disable */
...
var pinyinUtil = {
        ...
        if (dict.pinyin_dict_notone) {
            ...
            for (var i in pinyin_dict_notone) {
                ...
                for (var j = 0, len = temp.length; j < len; j++) {
                    var wordPinyin = dict.notone[temp[j]];
                    if (!wordPinyin) {
                        dict.notone[temp[j]] = i;
                    } else {
                        dict.notone[temp[j]] = `${wordPinyin} ${i}`;
                    }
                }
            }
        }
        // 如果导入了 pinyin_dict_withtone.js
        if (dict.pinyin_dict_withtone) {
    ...
    getPinyin: function(chinese, splitter, withtone, polyphone) {
        ...
        if (dict.withtone) {
            ...
            for (var i = 0, len = chinese.length; i < len; i++) {
                ...
                if (pinyin) {
                    ...
                    //空格，把noChinese作为一个词插入
                    noChinese && (result.push(noChinese), (noChinese = ''));
                    result.push(polyphone ? pinyin : pinyin.split(' ')[0]);
        ...
        if (!polyphone) return result.join(splitter);
            ...
            if (dict.pinyin_dict_polyphone) return parsePolyphone(chinese, result, splitter, withtone);
...
function parsePolyphone(chinese, result, splitter, withtone) {
    var poly = dict.pinyin_dict_polyphone;
...
pinyinUtil.parseDict({pinyin_dict_firstletter, pinyin_dict_notone});
